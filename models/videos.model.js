...
class videos {
    ...
    adiciona(videos, res) {
        ...
        if(videos.categoriaId == null || videos.categoriaId == "") {
            videos.categoriaId = 1;
        }
    ...
    buscaPorCategoria(categoria, res) {
        ...
        const sql= `select v.id,v.categoriaId,v.titulo,v.descricao,v.url,c.titulo as categoria from videos v , categorias c where v.categoriaId = c.id and c.titulo = "${categoria}" group by v.titulo order by v.id`
        console.log("SQL =>",sql);
        conexao.query(sql, (erro, resultados) => {
            if (erro) {
                res.status(400).json(erro)
